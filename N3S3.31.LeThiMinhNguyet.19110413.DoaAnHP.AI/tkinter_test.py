...
import sys
import time
from Eight_Puzzle_Final import *
...
#đưa hình ảnh xếp ngẫu nhiên khi click nút random
def set_text(num):
...
#hình ảnh và trạng thái trở lại ban đầu
def clearButton():
	for i in range(3):
		for j in range(3):
			listBt[i][j]['text']=''
...
#hiện thị phương pháp giải ở cuối màn hình
def setLabel():
...
#hoán đổi nút ô trống với nút kế bên 
...
#di chuyển theo phương pháp giải
def move():
	indx = 0
	indy=0
	step=''
...
#tìm phương pháp từ một số đưa vào
def findSolution(num):
	global solution
	solution, time =  final(tuple(num))
	lbNextstep['text']=solution[0]
	lbTime['text'] = "{:.5f}".format(float(time))
	setLabel()


#click nút Ok để tìm giải pháp
def ClickOK():
	clearButton()
...
#nút Random sẽ làm mất trạng thái hiện tại và tạo ra trạng thái mới của các mảnh ghép và cách giải
def ClickAuto():
	clearButton()
...
#thêm tiện ích như màu sắc, kích thước của các nút
...
lbNextstep = tk.Label(rightFrame)
